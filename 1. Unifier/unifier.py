...
parser.add_option("-t", "--threads", action="store", dest="threads", default="5", help="Amout of threats that can be used")
parser.add_option("-x", "--lines", action="store", dest="linesPerThread", default="10", help="Max lines per thread")
...
with open(options.log) as fileobject:
	for index, line in enumerate(fileobject, startIndex):
		...
		if index % int(float(options.linesPerThread)) == 0 or index >= num_lines or index >= endIndex - 1:
